...
<html>
  ...
  <article class="page-container">
	<form action="<%=basePath%>adminExtra/updateCycleImg" class="form form-horizontal" id="Form" method="post" enctype="multipart/form-data">
		<input type="hidden" name="imgId" value="${cycleImage.imgId}" />
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>图片名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" name="imgName" size="50" datatype="*" nullmsg="请输入图片名称" value="${cycleImage.imgName}"/>
			</div>
		</div>
			<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
						<div class="row clearfix">
							<div class="col-md-12 column">
								<img src="<%=basePath%>${cycleImage.imgAddress}"  class="img-circle" style="width: 200px; height: 200px;" />
							</div>
						</div>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
			     <input type="radio" name="imgType" value="1" ${cycleImage.imgType==1?"checked=\"checked\"" : "" }/>上
			     <input type="radio" name="imgType" value="2" ${cycleImage.imgType==2?"checked=\"checked\"" : "" }/>中
			     <input type="radio" name="imgType" value="3" ${cycleImage.imgType==3?"checked=\"checked\"" : "" }/>下
	    ...
	    <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				<option value="${car.carId}" ${car.carId==cycleImage.carId?'selected':''} >${car.carName}</option>
