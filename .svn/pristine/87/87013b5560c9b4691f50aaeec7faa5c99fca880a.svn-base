...
<html>
  ...
  <article class="page-container">
	<form action="<%=basePath%>adminRepository/updateRepository" class="form form-horizontal" id="Form" method="post">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" name="repName" size="50" datatype="*" nullmsg="请输入仓库名称" value="${rep.repName}"/>
			</div>
		</div>
	    <div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>库存：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" name="repStock" size="50" datatype="*" nullmsg="请输入库存数量" value="${rep.repStock}"/>
			</div>
		</div>
	      ...
	      <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select name="disId" id="disId">
				 ...
				 <option value="${dis.disId}" ${dis.disId==rep.disId?'selected':'' }>${dis.disName}</option>
				 </c:forEach>
				</select>
			</div>
		</div>
