...
<html>
  ...
  <article class="page-container">
	<form action="<%=basePath%>adminManage/updateManageUser" class="form form-horizontal" id="Form" method="post" enctype="multipart/form-data">
		<input type="hidden" name="manageId" value="${mUser.manageId}" />
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>登录名：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" name="loginName" size="50" datatype="*" nullmsg="请输入登录名" value="${mUser.loginName }"/>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>密码：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="password" class="input-text" name="loginPwd" size="50" datatype="*" errormsg="格式不正确" nullmsg="请输入密码" value="${mUser.loginPwd}"/>
			</div>
		</div>
	    ...
	    <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" name="manageName" size="50" datatype="*" nullmsg="请输入姓名" value="${mUser.manageName}"/>
			</div>
		</div>
	    <div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>性别：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="radio" name="manageSex" value="1" ${mUser.manageSex==1?"checked=\"checked\"" : "" }/>男
				<input type="radio" name="manageSex" value="2"${mUser.manageSex==2?"checked=\"checked\"" : "" }/>女
			</div>
		</div>
	    <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				onclick="WdatePicker()" value="${mUser.manageBirthday}"/>
			</div>
		</div>
	    ...
	    <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				datatype="*" nullmsg="请输入联系电话" value="${mUser.manageTel}"/>
			</div>
		</div>
	    <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				size="50" datatype="*" nullmsg="请输入身份证号码" value="${mUser.manageIdcard}"/>
			</div>
		</div>
	    <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				nullmsg="请输入邮件" value="${mUser.manageEmails}"/>
			</div>
		</div>
	  <div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>是否停用：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="radio" name="manageStatus" value="1"${mUser.manageStatus==1?"checked=\"checked\"" : "" }/>正常
				<input type="radio" name="manageStatus" value="2"${mUser.manageStatus==2?"checked=\"checked\"" : "" }/>已停用
			</div>
		</div>
			...
			<div class="formControls col-xs-8 col-sm-9">
						<div class="row clearfix">
							<div class="col-md-12 column">
								<img src="<%=basePath%>${mUser.manageImg}"  class="img-circle" style="width: 200px; height: 200px;" />
							</div>
						</div>
			</div>
		</div>
